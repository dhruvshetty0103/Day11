...
class Distance
{
	...
	double read()//for reading points
	{
	...
	double Distance_Calculation(double x1,double y1,double x2,double y2)//distance calculation
	{
		
		return (double)Math.sqrt(Math.pow(x2-x1,2)+Math.pow(y2-y1,2));//returning the calculated value
	}
}
...
public class Line_Problem 
{
	...
	{
		Distance d=new Distance();//Object of distance class created
		...
		Double l1=new Double(length_of_line1);//Double object created so that equals and compareTo functions can be used
		...
		if(l1.equals(l2))//when both are equal it returns 1
		...
		else
		{
			...
			else//else the value of result is -1 as for equality equals is used
			{
